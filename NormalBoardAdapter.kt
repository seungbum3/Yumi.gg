...
class NormalBoardAdapter(private val context: Context, private val posts: List<Post>) :
    RecyclerView.Adapter<NormalBoardAdapter.PostViewHolder>() {

    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): PostViewHolder {
        val view = LayoutInflater.from(context).inflate(R.layout.item_post, parent, false)
        val params = view.layoutParams as RecyclerView.LayoutParams
        // 아이템 크기 설정 (여기서 높이를 120dp로 설정)
        val height = (85 * context.resources.displayMetrics.density).toInt()  // 120dp를 픽셀로 변환
